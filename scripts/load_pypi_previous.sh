...
gsutil ls "gs://clickhouse-pypi/file_downloads/incremental/${midnight_utc_epoch}-*.parquet" | sed 's|gs://|https://storage.googleapis.com/|' > pypi-${midnight_utc_epoch}.txt

python queue_files.py --host ${CLICKHOUSE_HOST} --port 8443 --username default --password ${CLICKHOUSE_PASSWORD} --file "pypi-${midnight_utc_epoch}.txt" --task_database default --task_table tasks --files_chunk_size_min 500 --files_chunk_size_max 1000