...
export default function Line ({data, onSelect}) {
        ...
        tooltip: {
            ...
            formatter: (params) => {
                return `<div class="${styles.tooltip}">
                    <span class="${styles.tooltiptext}">${Number(params[0].value).toLocaleString("en-US")}</span>
                </div>`
            },
            extraCssText: 'visibility: hidden;padding:0px;',
            position: (point, params, dom, rect, size) => {
                const echartsInstance = chartRef.current.getEchartsInstance();
                const pos = echartsInstance.convertToPixel({ seriesIndex: 0 }, [params[0].axisValue, params[0].value]);
    ...
    const onBrushEnd = (params) => {
        if (params.areas.length > 0) {
            ...
            onSelect && xAxis[start] && xAxis[end] && onSelect(xAxis[start], xAxis[end])
        }
    }
    ...
    return <div className="rounded-lg bg-chart border-2 border-slate-500 h-full justify-between flex flex-col" onMouseMove={onMouseOver} onMouseOut={onMouseOut}>
            <ReactECharts ref={chartRef} option={options} style={{ width: "100%", height: "100%" }} lazyUpdate={true} onEvents={{
                'brushEnd': onBrushEnd
            }}/>
    </div>;
}
