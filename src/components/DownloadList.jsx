...
async function DownloadList({ package_name, version, max_date, min_date, country_code, type, ...props }) {
  ...
  return (
    <div {...props}>
      <div className='rounded-lg bg-slate-850 flex items-center gap-8 justify-between p-4 h-24 border border-slate-700 rounded-l'>
        <div className='h-16 w-16 flex gap-4'>
          <Image className='h-16 w-16 min-w-[64px] min-h-16' src='/download.svg' alt='Download' width={16} height={16} />
